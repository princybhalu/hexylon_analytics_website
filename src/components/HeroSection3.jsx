    ...
    const animate = () => {
      ...
      activeSets.current.forEach(set => {
        ...
        set.points.forEach((point, index) => {
          ...
          const floatOffset = Math.sin(frameCount.current * 0.02 + set.phase) * 2;
          const rotationOffset = Math.sin(frameCount.current * 0.01 + set.phase) * 0.5;
          
          point.x = point.originalX + Math.cos(point.angle + rotationOffset) * floatOffset;
          point.y = point.originalY + Math.sin(point.angle + rotationOffset) * floatOffset;
      ...
      activeSets.current.forEach(set1 => {
        activeSets.current.forEach(set2 => {
          ...
          if (distance < 200) {
            ...
            if (Math.random() < 0.01) { // Occasional flashing connection
              ctx.beginPath();
              ctx.moveTo(point1.x, point1.y);
              ctx.lineTo(point2.x, point2.y);
