...
const TypingEffect = ({ text, triggerAnimation, onComplete }) => {
  ...
  }, [triggerAnimation, text, isTyping, onComplete]);
...
const WorkCard = ({ title, description, number }) => {
  ...
  return (
    <div 
      ref={cardRef}
      className="flex-shrink-0 w-[400px] h-[300px] bg-white rounded-xl shadow-lg p-6 transform transition-all duration-500"
    >
...
const HowWeWorkSection = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(cardsContainerRef.current, { autoAlpha: 0, y: 50 });
      
      // Center header animation on scroll
      ScrollTrigger.create({
        ...
        onEnter: () => {
          gsap.to(headerRef.current, {
            duration: 1,
            y: "50vh",
            onComplete: () => setTriggerTyping(true)
          });
        }
      });
    });
  ...
  return (
    <div ref={sectionRef} className="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-20">
        {/* Header Section */}
        <div 
          ref={headerRef} 
          className="flex flex-row items-center gap-1 mb-16 transform translate-y-0"
        >
          <DropletAnimation />
          <TypingEffect 
